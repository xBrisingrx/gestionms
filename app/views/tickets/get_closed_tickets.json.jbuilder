json.data @closed_tickes do |t|
  ...
  json.title t.title
  json.actions " <a class='btn btn-mini btn-info' href='tickets/#{t.id}'>Ver</a> " +
    if t.state_ticket != 'closed'
               "<button class='btn btn-mini btn-success close_ticket_btn' data-id='#{t.id}'"+
               "data-toggle='modal' data-target='#modal_close_ticket'> <i class='fa fa-check'></i></button>"
    else
               "<button class='btn btn-mini u-btn-indigo'> Cerrado </i></button>"
    end
end